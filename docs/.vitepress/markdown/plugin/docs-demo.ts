...
// https://www.cnblogs.com/wangyongcun/p/16805041.html
...
const docsDemoComponent = 'DocsDemo';
...
function genDemo(meta, md: MarkdownIt) {
  ...
  let htmlOpenString = `<${docsDemoComponent}`;
  // <docsDemoComponent
  let attrsStr = '';
  if (meta.title) {
    attrsStr += ` title="${meta.title}"`;
    // <docsDemoComponent title="基本使用"
  }
  if (meta.desc) {
  ...
  if (meta.language) {
    (srcFile as any).language = meta.language;
  }
  ...
  if (!srcFile) {
    ...
    if (useIframeMode) {
      ...
      // <docsDemoComponent title="基本使用" ><iframe src="/-demos/${localName}.html"`
      ...
      // <docsDemoComponent title="基本使用" ><Demo1002 />
...
export default (md: MarkdownIt) => {
  md.renderer.rules.html_inline = md.renderer.rules.html_block = (tokens, idx) => {
    ...
    if (/^<demo(?=(\s|>|$))/i.test(content.trim())) {
      ...
      return `${demoData.htmlOpenString}</${docsDemoComponent}>`;
    } else {
      return content;
    }
  };
  ...
  md.use(mdContainer, 'demo', {
    ...
    render(tokens, idx) {
      if (tokens[idx].nesting === 1 /* means the tag is opening */) {
        ...
        return `</template></${docsDemoComponent}>`;
      }
    },
  });
};