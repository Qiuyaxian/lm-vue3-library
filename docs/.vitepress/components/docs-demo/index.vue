    ...
    <section v-bind="$attrs" class="demo-container">
      ...
      <div class="demo-actions">
        ...
        <div class="theTab" v-if="sourceVisible && files.length > 1">
          <span v-for="file in files" :key="file.name" :title="file.name" @click="currentTab = file.name"
      ...
      <template v-if="sourceVisible">
        <div v-if="currentFile?.htmlStr" v-html="decodeURIComponent(currentFile.htmlStr)" key="highlight"
          :class="`language-${currentFile.language} source-code-wrapper`" />
        <div v-else-if="currentFile?.codeStr" key="raw" :class="`language-${currentFile.language} source-code-wrapper`"
          style="color: white">
